  ...
  export default {
    ...
    computed: {
      ...
      cssStyle () {
        return eXo.$applicationUtils.getStyle(this.container, {
          isApplicationBackground: this.container.template === eXo.$layoutUtils.bannerCellTemplate,
      ...
      isCell () {
        return this.container.template === eXo.$layoutUtils.cellTemplate;
      },
      sectionType () {
        if (this.$root.isSiteLayout) {
          return this.container.template;
        } else {
          const section = eXo.$layoutUtils.getSection(this.$root.layout, this.parentId);
          return section?.template || 'page';
        }
      },
      isDraggableCell () {
        return this.container.template === eXo.$layoutUtils.cellTemplate
          || this.container.template === eXo.$layoutUtils.bannerTemplate
          || this.container.template === eXo.$layoutUtils.sidebarCellTemplate;
      },
      dragSelectionClass () {
    ...
    methods: {
      ...
      startMoving () {
        ...
        const section = eXo.$layoutUtils.getContainerById(this.$root.layout, this.parentId);
        this.$root.movingParentDynamic = section?.template === eXo.$layoutUtils.flexTemplate;
      },
      endMoving (event) {
        this.dragged = false;
        this.$root.movingParentId = null;

        const fromCell = eXo.$layoutUtils.getContainerById(this.$root.layout, event.from.getAttribute('data-storage-id'));
        const toCell = eXo.$layoutUtils.getContainerById(this.$root.layout, event.to.getAttribute('data-storage-id'));
        const application = eXo.$layoutUtils.getContainerById(this.$root.layout, event.item.getAttribute('data-storage-id'));

        if (fromCell && toCell && application) {
          if (this.isCell) {
            const section = eXo.$layoutUtils.getSectionByContainer(this.$root.layout, event.item.getAttribute('data-storage-id'), this.$root.isSiteLayout);
