...
<script>
  export default {
    ...
    methods: {
      moveUpChildNode (navigationNodeId) {
        if (this.navigationNode.children.length) {
          ...
          if (index !== -1) {
            ...
            eXo.$navigationLayoutService.moveNode(navigationNodeId, null, previousNodeId).then(() => {
      ...
      formatDate (time) {
        return !time && '--' || eXo.$dateUtil.formatDateObjectToDisplay(new Date(time),this.fullDateFormat, this.lang);
      },
    },
  };
</script>