    ...
    <exo-drawer
      ...
      <template v-if="drawer" #content>
        <v-form
          ...
          <v-card-text v-if="visible && isScheduled" class="pt-0">
  ...
  export default {
    ...
    methods: {
      ...
      open (parentNavigationNode) {
        ...
        if (this.editMode) {
          ...
          if (this.navigationNode?.pageKey) {
            ...
            eXo.$pageLayoutService.getPage(pageRef)
              .then(page => {
                this.selectedPage = {
                  pageRef,
      ...
      createNode (previousNodeId, pageData, startScheduleDate, endScheduleDate, nodeLabels) {
        eXo.$navigationLayoutService.createNode(this.navigationNode.id, previousNodeId, this.nodeLabel, this.nodeId, this.nodeIcon, this.visible, this.isScheduled, startScheduleDate, endScheduleDate, nodeLabels?.labels, pageData?.pageRef, pageData?.pageRef && pageData?.nodeTarget || 'SAME_TAB')
          .then(createdNode => {
      ...
      updateNode (pageData, pageRef, startScheduleDate, endScheduleDate, nodeLabels) {
        eXo.$navigationLayoutService.updateNode(this.navigationNode.id, this.nodeLabel, pageRef, this.visible, this.isScheduled, startScheduleDate, endScheduleDate, nodeLabels?.labels, pageData?.nodeTarget || this.navigationNode.target, this.nodeIcon)
          .then(() => {
            this.$root.$emit('refresh-navigation-nodes');
            this.$root.$emit('close-add-element-drawer');
            this.close();
      ...
      updatePageLink (startScheduleDate, endScheduleDate, nodeLabels) {
        ...
        eXo.$pageLayoutService.updatePageLink(pageRef, this.link)
          .then(() => {
            const pageData = {
              pageRef,
              'nodeTarget': this.nodeTarget,
      ...
      getNodeLabels () {
        eXo.$navigationLayoutService.getNodeLabels(this.navigationNode.id)
          .then(data => {
      ...
      openTargetPage (pageData, nodeId) {
        if (pageData?.pageRef) {
          if (pageData?.pageType === 'PAGE' && pageData?.pageRef) {
            return eXo.$pageLayoutService.editPageLayout(nodeId || this.nodeId, pageData?.pageRef);
