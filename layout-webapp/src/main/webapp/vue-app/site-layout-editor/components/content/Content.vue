    ...
    methods: {
      ...
      switchDisplayMode () {
        if (this.$root.displayMode === 'mobile') {
          eXo.$layoutUtils.applyMobileStyle(this.$root.layout);
        } else {
          eXo.$layoutUtils.applyDesktopStyle(this.$root.layout);
        }
      },
      async setLayout (layout) {
        ...
        const compatible = !!this.$root.layout || eXo.$layoutUtils.parseSite(layout);
        if (this.$root.layout) {
      ...
      handleAddApplication (application) {
        try {
          this.addHistory();
          const container = eXo.$layoutUtils.getContainerById(this.$root.layout, this.$root.selectedSectionId);
          eXo.$layoutUtils.newApplication(container, application, true);
          this.saveDraft();
        } finally {
      ...
      handleDeleteApplication (sectionId, container) {
        const section = eXo.$layoutUtils.getContainerById(this.$root.layout, sectionId);
        if (section) {
          this.addHistory();
      ...
      handleEditApplication (sectionId, container, applicationCategoryTitle, applicationTitle) {
        const section = eXo.$layoutUtils.getContainerById(this.$root.layout, sectionId);
        const containerToEdit = section.template === eXo.$layoutUtils.bannerCellTemplate ? section : container;
      ...
      saveDraft (layout) {
        ...
        this.loading++;
        const layoutToUpdate = eXo.$layoutUtils.cleanAttributes(layout || JSON.parse(JSON.stringify(this.$root.layout)), false, true);
        return eXo.$siteLayoutService.updateSiteLayout(this.$root.draftSiteType, this.$root.draftSiteName, layoutToUpdate, 'contentId')
          .then(layout => {
